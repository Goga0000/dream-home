document.addEventListener('DOMContentLoaded', () => {
   // --- 1. Open modal if `open_modal=1` в URL ---
   (() => {
      const params = new URLSearchParams(window.location.search);
      if (params.get('open_modal') === '1') {
         const wrapper = document.querySelector('.filter-modal-wrapper');
...
(() => {
   ...
   const setActiveMarker = (idx = null, scrollSwiper = false) => {
      ...
      markers.forEach((marker, i) => {
         marker.content.style.border = i === idx ? "3px solid #4C4DDC" : "3px solid white";
   ...
   const updateMarkers = () => {
      ...
      items.forEach((item, i) => {
         ...
         Object.assign(markerContent.style, {
            width: "48px", height: "48px", border: "3px solid white", borderRadius: "50%", overflow: "hidden",
            background: "#fff", boxShadow: "0 2px 8px rgba(76,77,220,0.16)", display: "flex", alignItems: "center", justifyContent: "center",
         ...
         const marker = new google.maps.marker.AdvancedMarkerElement({
            map, position: { lat, lng }, content: markerContent, title
         });
   ...
   // Анимация и подсчет высоты bar graph (без range slider)
   const animateBarGraph = () => {
      ...
      const animateHeight = (el, to, d = 500) => {
   ...
   const waitForMapInit = (retries = 20) => {
      ...
      // Запуск анимации bar graph после полной инициализации карты
      animateBarGraph();
   };
